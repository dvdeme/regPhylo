...
GetSeqInfo_NCBI_taxid = function(splist = NULL, gene = NULL, filename = NULL) {
    ...
    for (k in 1:dim(splist)[1]) {
        ...
        print(paste(as.character(splist[k, 2]), ": ", length(ee$req), "seq."))
        ...
        if (is.null(ee$nelem) == FALSE)
            {

                AccNb = seqinr::getName(ee)
                imax = length(AccNb)
                for (i in 1:imax) {
                  # Loop over the sequences within a species.
                  Infot = vector()
                  # Provide the taxa name provided by the list
                  Infot = c(Infot, as.character(splist[k, 2]), AccNb[i])
                  if (seqinr::getLength(ee$req[[i]]) > 5000) {
                    ...
                    Infot = c(Infot, seqinr::c2s(seqinr::getSequence(ee$req[[i]])))
                  }
                  Infot = c(Infot, seqinr::getLength(ee$req[[i]]))
