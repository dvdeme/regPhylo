...
#' from NCBI and BOLD databases from a list fo taxa of interest. Pipeline.DNA.extraction
#' function is deprecated see Pipeline.DNA.extraction.V2.
...
#' only the names that will be reported in the output table as 'species_name'.
#' 
#' @param Path.BOLD name of the data.frame (R object) containing the Barcode Of Life Database BOLD, #' it can be downloaded at https://boldsystems.org/data/data-packages/
...
#' @param Steps is a vector of numeric value from 1 to 10, with
#' Step 1 = Extract the DNA sequence from NCBI,
#' Step 2 = Extract the DNA sequence from BOLD
#' Step 3 = Merge and dereplication of GenBank and BOLD Data.
#' Step 4 = Homogeneize the Geographic information related to the sequences.
#' Step 5 = Improve the geographic information (might not be very reliable).
#' Step 6 = Build the Species by gene matrix
#' Step 7 = If some species were excluded from the NCBI search because there were too many sequences, we can extract the sequence from the selected gene regions of interest only for those taxa. remove duplicated record, remove record without DNA sequences, and homogeneize the geographic coordinate, imporve the geograhic coordinate and rebuild the species by gene matrix)
#' Step = 8 Select the gene of interest,based on the Species by gene matrix produced dring step 6 or 7.
#' Step = 9 Export all DNA sequences and metadata for the selected gene regions. Extract all the sequences (including from long annotated DNA sequences) and associated information.
#' Step = 10 First alignment of all sequences and detection of sequences that should be reverse complemented.
...
#' @export Pipeline.DNA.extraction.V2





Pipeline.DNA.extraction.V2 = function(input.ncbi = NULL,
                                      input.bold = NULL,
                                      Path.BOLD = NULL,
                                      gene = "ALL",
                                      max.seq = 1000,
  ...
  # Step 1                         
  if(is.na(match(1, Steps)) == FALSE) {
    
    if(is.na(match("Raw.Data", list.files(Path.output))) == TRUE){
      dir.create(paste(Path.output, "/Raw.Data", sep = ""))
    }
    ...
    # Export a table reporting the number of sequence present in NCBI.
    write.table(as.data.frame(NCBI.nb.seq), file = paste(Path.output.raw, "NCBI.NbSequences.txt", sep = ""), 
                sep = "\t", row.names = F)
    
    # 3_Extract the DNA sequence and Metadata from GenBank.
    NCBI.stats = GetSeqInfo_NCBI_taxid(
      splist = input.ncbi.short,
      gene = gene,
      filename = paste(Path.output.raw, "Seq.NCBI.txt", sep = "")
    ...
    if(Remove.Large.Nuc.Fragment == TRUE){
      ...
      if(length(a)> 0){
        ...
        # utils::write.table(NCBI.data[-a,], file = paste(Path.output.raw, "Seq.NCBI.txt", sep = ""),
        #                   sep = "\t")
        utils::write.table(NCBI.data[a,], file = paste(Path.output.raw, "Seq.NCBI.Rm.WholeGenome.txt", sep = ""),
                           sep = "\t")
        NCBI.data =  NCBI.data[-a,]
      }
      # Convert the sequence length into numeric values
      ...
      if(length(b) > 0){
        ...
        # utils::write.table(NCBI.data, file = paste(Path.output.raw, "Seq.NCBI.txt", sep = ""),
        #                   sep = "\t")
        utils::write.table(NCBI.data[b,], file = paste(Path.output.raw, "Seq.NCBI.Rm.LongSeq.txt", sep = ""),
                           sep = "\t")
        NCBI.data =  NCBI.data[-b,]
    ...
    # If the last Step performed is the Steps 1 
  ...
  # Step 2
  if(is.na(match(2, Steps)) == FALSE) {
    
    # Test the presence of the Raw.Data folder.
  ...
  # Step 3
  if(is.na(match(3, Steps)) == FALSE) {
    
    # Need to load the NCBI data if step 1 was not performed during the same run of the pipeline
    if(is.na(match(1, Steps)) == TRUE){
      NCBI.data = read.delim(
        paste(Path.output.raw, "Seq.NCBI.txt", sep = ""),
        sep = "\t",
        header = T)
    ...
    # Need to load the BOLD data if step 1 was not performed during the same run of the pipeline
    if(is.na(match(2, Steps)) == TRUE){ 
      BOLD.data = read.delim(
        paste(Path.output.raw, "Seq.BOLD.txt", sep = ""),
        sep = "\t",
        header = T)
    } 
  ...
  # Step 4
  if(is.na(match(4, Steps)) == FALSE) {
    
    if(is.na(match(3, Steps)) == TRUE){
      AllSeqDF = read.delim(
        paste(Path.output.raw, "Seq_NCBI_BOLD.txt", sep = ""),
        sep = "\t",
        header = T)
    }
  ...
  # Step 5
  if(is.na(match(5, Steps)) == FALSE) {
    
    if(is.na(match(4, Steps)) == TRUE){
      AllSeqDF1 = read.delim(
        paste(Path.output.raw, "Seq_NCBI_BOLD_Geo1.txt", sep = ""),
        sep = "\t",
        header = T)
    }
  ...
  # Step 6
  if(is.na(match(6, Steps)) == FALSE) {
    
    if(is.na(match(5, Steps)) == TRUE){
      AllSeqDF2 = read.delim(
        paste(Path.output.raw, "Seq_NCBI_BOLD_Geo2.txt", sep = ""),
        sep = "\t",
        header = T)
    }
    
    
    # Build the Species by gene matrix (That's very Important !!).
    Sp.DNA.Mat.naive = SpeciesGeneMat.Bl(
      input = AllSeqDF2,
      output = paste(Path.output.raw, "Matrix.Sp.DNA.Naive", sep = ""))
    
    # If the last Step performed is the Steps 6 (Species by gene matrix without 
    if(Steps[length(Steps)] == 6){
      results = Sp.DNA.Mat.naive  
      return(results)
    }
  ...
  # Step 7
  if(is.na(match(7, Steps)) == FALSE) {
    
    
    # check if the NCBI.nb.seq is present in the global Envir. of if present in the Raw.Data folder
    if("NCBI.nb.seq" %in% ls(envir = .GlobalEnv) == FALSE){ 
      if(is.na(match("NCBI.NbSequence.txt" , list.files(Path.output.raw)))== FALSE){
        stop("Step 7 cannot be completed as the NCBI.NbSequences.txt file cannot be found in the Raw.Data folder" )
      } else {
        NCBI.nb.seq = read.delim(paste(Path.output.raw, "NCBI.NbSequences.txt", sep = ""), sep = "\t", h = T)
      }
    }
    ...
    # 9_If some species were excluded from the NCBI search because there had too many sequences, we can extract the sequences from the selected gene regions of interest only for those taxa.
    ...
    if (length(a) > 0) {
      ...
      if("Sp.DNA.Mat.naive" %in% ls(envir = .GlobalEnv) == FALSE){ 
        Sp.DNA.Mat.naive = read.delim(paste(Path.output.raw, "Matrix.Sp.DNA.NaiveSpDNA_Mat.txt", sep = ""), sep = "\t", h=T)
        Sp.DNA.Mat.naive.2 = read.delim(paste(Path.output.raw, "Matrix.Sp.DNA.NaiveSummary_DNApers.txt", sep = ""), 
                                        sep = "\t", h=T)
        Sp.DNA.Mat.naive = list(Species.Gene_matrix = Sp.DNA.Mat.naive, Summary_DNA = Sp.DNA.Mat.naive.2)
      } 
      ...
      if (!is.na(co1.pres)) {
        ## include additional gene nomenclature for the CO1.
        gene.listB = c(gene.listB, "CO1", "COX1", "COXI",
                       "cytochrome c oxidase subunit I", "Cytochrome c oxidase subunit I","Cytochrome oxidase subunit I",
                       "Cytochrome c oxydase subunit I","Cytochrome oxydase subunit I",
                       "cytochrome oxydase subunit I","cytochrome c oxydase subunit I") ### Need to add additional gene names
        
      }
      
      # Remove potential gene noted NA in the gene selection.
      if(length(which(is.na(gene.listB))) > 0){
        gene.listB = gene.listB[-which(is.na(gene.listB))]
      }
      ...
      gene.listB.export = unique(tolower(NCBI.nb.seq.V2[which(as.numeric(NCBI.nb.seq.V2[, 2]) > 0), 1]))
      # Replace by tolower and remove space
      #gene.listB.export = tolower(gsub(" ", "", gsub(
      #  "S ribosomal RNA", "SRRNA", gene.listB.short
      #)))
      ...
      # Export the new DNA sequences.
      NCBI.data2 = do.call(rbind, lapply(1:length(gene.listB.export), function(x) {
        NCBI.stats2 = GetSeqInfo_NCBI_taxid(
          splist = input.ncbi.short2,
          gene = gene.listB.export[x],
          filename = paste(
            Path.output.raw,
            "Seq.NCBI.",
            gene.listB.export[x],
      ...
      # Merge and dereplication of GenBank and BOLD Data. # here we need to add the BOLD data as well, because the sequence might be already present in BOLD eventhough we did not exracted then fromNCBI the first time!!!
      
      AllSeqDF.V2 = Congr.NCBI.BOLD.perReposit.V2(
        input.NCBI = NCBI.data2,
        input.BOLD = NULL,
        output = paste(Path.output.raw, "Seq_NCBI.Congr.Addi.txt", sep = ""),
        input.perReposit = NULL
      )
      ...
      AllSeqDF1.V2 = GeoCoord.WGS84(
        input = AllSeqDF.V2,
        output = paste(Path.output.raw, "Seq_NCBI.Congr_Geo1.Addi.txt", sep = "")
      )
      
      # Improve the geographic information (might not be very reliable, but you do not care really).
      AllSeqDF2.V2 = GeoCodeName(
        input = AllSeqDF1.V2,
        output = paste(Path.output.raw, "Seq_NCBI.Congr_Geo2.Addi.txt", sep = ""),
        AutoCorrNZ = FALSE
      )
      
      
      # test the presence of AllSeqDF2 in the R. GlobEnv
      if("AllSeqDF2" %in% ls(envir = .GlobalEnv) == FALSE){ 
        AllSeqDF2 = read.delim(
          paste(Path.output.raw, "Seq_NCBI_BOLD_Geo2.txt", sep = ""),
          sep = "\t", header = T)
      }
      ...
      #### Need to remove all the potential duplicated sequences 
      ID.DF2 = paste(AllSeqDF2$TaxaName, AllSeqDF2$AccessNCBI, sep = "|")
      ID.DF2.V2 = paste(AllSeqDF2.V2$TaxaName, AllSeqDF2.V2$AccessNCBI, sep = "|")
      
      if(length(intersect(ID.DF2, ID.DF2.V2) %in% ID.DF2.V2)  > 0){
        AllSeqDF2.V2.unique = AllSeqDF2.V2[-match(intersect(ID.DF2, ID.DF2.V2), ID.DF2.V2),]
        
        # Add the first and the additional NCBI data together
        AllSeqDF3 = as.data.frame(rbind(AllSeqDF2, AllSeqDF2.V2.unique))
      } else {
        # Add the first and the additional NCBI data together
        AllSeqDF3 = as.data.frame(rbind(AllSeqDF2, AllSeqDF2.V2))
      }
    ...
    # If the last Step performed is the Steps 7 (Species by gene matrix including the taxa with too many DNA sequence to be extracted during the first step).
    if(Steps[length(Steps)] == 7){
      results = Sp.DNA.Mat.naive  
      return(results)
    }
  } # end of step 7
  ...
  # Step 8
  if(is.na(match(8, Steps)) == FALSE) {
    
    
    # Test the presence of Sp.DNA.Mat.naive in the R. GlobEnv
    if("Sp.DNA.Mat.naive" %in% ls(envir = .GlobalEnv) == FALSE){ 
      Sp.DNA.Mat.naive = read.delim(paste(Path.output.raw, "Matrix.Sp.DNA.NaiveSpDNA_Mat.txt", sep = ""), sep = "\t", h=T)
      Sp.DNA.Mat.naive.2 = read.delim(paste(Path.output.raw, "Matrix.Sp.DNA.NaiveSummary_DNApers.txt", sep = ""), 
                                      sep = "\t", h=T)
    ...
    if(length(which(is.na(GeneSelection))) > 0){
      GeneSelection = GeneSelection[-which(is.na(GeneSelection))]
    }
    
    
    saveRDS(GeneSelection, file = paste(Path.output.raw, "GeneSelection.rds", sep = ""))
    
    
    if(Steps[length(Steps)] == 8){
      results = c(Sp.DNA.Mat.naive,  list(GeneSelection = GeneSelection))
      return(results)
    }
  ...
  # Step 9
  if(is.na(match(9, Steps)) == FALSE) {
    
    # 11_Export all DNA sequences and metadata for the selected gene regions
    # Extract all the sequences (including from long annotated DNA sequences) and
    # associated information.
    PoolSeq = read.delim(
      paste(Path.output.raw, "Matrix.Sp.DNA.Naive_CleanDataset.txt", sep = ""),
      sep = "\t", h = T, check.names = F)
    
    
    if(is.na(match("Selected.META", list.files(Path.output))) == TRUE){
      dir.create(paste(Path.output, "/Selected.META", sep = ""))
    }
    
    GeneSelection = readRDS(paste(Path.output.raw, "GeneSelection.rds", sep = ""))
    ...
    ## Create a folder to export the selected DNA alignments
    if(is.na(match("Selected.DNA", list.files(Path.output))) == TRUE){
      dir.create(paste(Path.output, "/Selected.DNA", sep = ""))
    }
    ...
    if(Steps[length(Steps)] == 9){
      results = c(list(DNA.Seq.And.Metadata.Selected.DNA = PoolAllSeq.Select.Genes),  
                  list(GeneSelection = GeneSelection))
      return(results)
    }
  ...
  # Step 10
  if(is.na(match(10, Steps)) == FALSE) {
    
    
    if(is.na(match("FirstAlign", list.files(paste(Path.output, "/Selected.DNA", sep = "")))) == TRUE){
      dir.create(paste(Path.output, "/Selected.DNA/FirstAlign", sep = ""))
    }
    
    if("PoolAllSeq.Select.Genes" %in% ls(envir = .GlobalEnv) == FALSE){ 
      PoolAllSeq.Select.Genes = read.delim(paste(Path.output, "/Selected.META/MetaData.Select.DNA.Select.DNA.txt",
                                                 sep = ""), sep = "\t", h=T)
    }
