...
#' @param Steps is a vector of numeric value from 1 to 3, with 1 extract the DNA squences and 
...
#' @param nthread, if steps 3 is present in the option Step (eg. Steps = c(1,2,3)) then we can choose #' how many thread we wouldlike to use to perform the first alignment steps in case there are multiple
#'  markers to align.
...
#' from a DNA perspective ("Matrix.Sp.DNA.NaiveSummary_DNApers.txt") and from a species perspective ("Matrix.Sp.DNA.NaiveSummary_SPECIESpers.txt").
...
Pipeline.DNA.extraction = function(input.ncbi = NULL, input.bold = NULL, gene = "ALL", max.seq = 10000, Path.output = NULL, Nb.DNA.marker = 1, Steps = c(1,2), nthread = 1, methods = "mafftfftns2", nthread.per.align = NULL){
...
NCBI.stats = GetSeqInfo_NCBI_taxid(splist = input.ncbi.short, gene = gene, filename = paste(Path.output.raw, "Seq.NCBI.txt", sep=""))

# Load the NCBI sequence and metadata in R
NCBI.data = read.delim(paste(Path.output.raw, "Seq.NCBI.txt", sep=""), sep = "\t", header = T)

# 4_Extract DNA sequences and Metadata from BOLD.

BOLD.data = GetSeq_BOLD(splist = input.bold, filename=paste(Path.output.raw, "Seq.BOLD.txt", sep=""), bold.id = FALSE)
BOLD.stats = as.data.frame(BOLD.data[[1]])

# 5_Merge and dereplication of GenBank and BOLD Data.
AllSeqDF = Congr.NCBI.BOLD.perReposit(input.NCBI=NCBI.data, input.BOLD=BOLD.data[[2]],
output= paste(Path.output.raw, "Seq_NCBI_BOLD.txt", sep = ""), input.perReposit=NULL)


# 6_Homogeneize the Geographic information related to the sequences. (not very important at that stage).
AllSeqDF1 = GeoCoord.WGS84(input=AllSeqDF, output=paste(Path.output.raw, "Seq_NCBI_BOLD_Geo1.txt", sep = ""))

# 7_Improve the geographic information (might not be very reliable, but you do not care really).
AllSeqDF2 = GeoCodeName(input=AllSeqDF1, output= paste(Path.output.raw, "Seq_NCBI_BOLD_Geo2.txt", sep = ""), AutoCorrNZ = FALSE)


# 8_Build the Species by gene matrix (That's very Important !!).
Sp.DNA.Mat.naive = SpeciesGeneMat.Bl(input=AllSeqDF2, output= paste(Path.output.raw, "Matrix.Sp.DNA.Naive", sep = ""))
...
gene.listB.export = tolower(gsub(" ", "", gsub("S ribosomal RNA", "SRRNA", gene.listB.short)))

# Export the new DNA sequences.
NCBI.data2 = do.call(rbind, lapply(1:length(GeneSelection),function(x){
NCBI.stats2 = GetSeqInfo_NCBI_taxid(splist = input.ncbi.short2[2:4,], gene = gene.listB[x], filename = paste(Path.output.raw, "Seq.NCBI.", gene.listB.export[x], ".txt", sep=""))
read.delim(paste(Path.output.raw, "Seq.NCBI.", gene.listB.export[x], ".txt", sep=""), sep = "\t", header = T)
}))
...
AllSeqDF.V2 = Congr.NCBI.BOLD.perReposit(input.NCBI=NCBI.data2, input.BOLD=NULL,
output= paste(Path.output.raw, "Seq_NCBI.Congr.txt", sep = ""), input.perReposit=NULL)

# Homogeneize the Geographic information related to the sequences. (not very important at that stage).
AllSeqDF1.V2 = GeoCoord.WGS84(input=AllSeqDF.V2, output=paste(Path.output.raw, "Seq_NCBI.Congr_Geo1.txt", sep = ""))

# Improve the geographic information (might not be very reliable, but you do not care really).
AllSeqDF2.V2 = GeoCodeName(input=AllSeqDF1.V2, output= paste(Path.output.raw, "Seq_NCBI.Congr_Geo2.txt", sep = ""), AutoCorrNZ = FALSE)


AllSeqDF3 = as.data.frame(rbind(AllSeqDF2, AllSeqDF2.V2))
# 8_Build the Species by gene matrix (That's very Important !!).
Sp.DNA.Mat.naive = SpeciesGeneMat.Bl(input = AllSeqDF3, output = paste(Path.output.raw, "Matrix.Sp.DNA.Naive", sep = ""))
...
if(match(2, Steps) == 2){ ## PERFORM THE STEPS 2
...
PoolSeq = read.delim(paste(Path.output.raw,"Matrix.Sp.DNA.Naive_CleanDataset.txt", sep = ""), sep="\t", h=T)
...
# Export the metadata
PoolAllSeq.Select.Genes = Select.DNA(input = PoolSeq, gene.list = GeneSelection, output = paste(Path.output.Select.Meta, "/MetaData.Select.DNA", sep = ""), timeout = 15, Seqinr = FALSE)
...
Export.AllSeq.5genes = SelBestSeq(input=PoolAllSeq.Select.Genes,
output=Path.output.Select.DNA, RefPoint=cbind(0, 0), 
Alignment=T, MaxSeq="ALL", gene.list = GeneSelection, SeqChoice="Median")

if(length(Steps) == 2){
results = c(list(DNA.Seq.And.Metadata.Selected.DNA = PoolAllSeq.Select.Genes), list(GeneSelection = GeneSelection), Sp.DNA.Mat.naive)
...
if(match(3, Steps) == 3){ ## PERFORM THE STEPS 3
...
Res.align = First.Align.All(input= input,
output= output,
nthread = nthread, methods = methods, nthread.per.align = nthread.per.align)

results = c(list(Alignment.log.file = Res.align), list(DNA.Seq.And.Metadata.Selected.DNA = PoolAllSeq.Select.Genes), list(GeneSelection = GeneSelection), Sp.DNA.Mat.naive)
return(results)
}
...
GetSeqNumber_NCBI_taxid = function(splist = NULL, gene = "ALL", chunk_size = 50) {
